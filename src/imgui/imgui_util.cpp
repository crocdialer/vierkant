...
void draw_scene_ui(const ScenePtr &scene, Object3DPtr &camera, std::set<vierkant::Object3DPtr> *selection)
{
    ...
    if(ImGui::BeginTabItem("lights"))
    {
        ...
        {
            auto &light = *assets->light(light_id);

            if(ImGui::TreeNode(&light, "%s", light.name.empty() ? "<unnamed>" : light.name.c_str()))
            {
                draw_light_ui(light);
                ImGui::Spacing();
                ImGui::Separator();
...
void draw_light_ui(vierkant::lightsource_t &light)
{
    ...
    // allow grabbing the light-id
    strcpy(text_buf, light.id.str().c_str());
    ImGui::InputText("light-id", text_buf, sizeof(text_buf), ImGuiInputTextFlags_ReadOnly);

    strcpy(text_buf, light.name.c_str());
    if(ImGui::InputText("name", text_buf, IM_ARRAYSIZE(text_buf), ImGuiInputTextFlags_EnterReturnsTrue))
    {
        light.name = text_buf;
    }
                  ...
                  vierkant::mesh_component_t &mesh_component)
{
    ...
    // materials
    if(!mesh->entries.empty() && ImGui::TreeNode("materials", "materials (%zu)", material_ids.size()))
    {
        for(uint32_t i = 0; i < material_ids.size(); ++i)
        {
...
void draw_transform_guizmo(const vierkant::Object3DPtr &object, const vierkant::Object3DConstPtr &camera,
                           GuizmoType type, GuizmoSpace space)
{
    if(camera && type != GuizmoType::INACTIVE)
    {
        auto transform = object->global_transform();
        if(draw_transform_guizmo(transform, camera, type, space)) { object->set_global_transform(transform); }
    }
}
                           ...
                           GuizmoType type, GuizmoSpace space)
{
    if(object_set.size() > 1)
    {
        ...
        if(type == GuizmoType::TRANSLATE)
        {
            ...
            if(draw_transform_guizmo(transform, camera, type, space))
            {
