...
void main()
{
    ...
    if(!context.disable_material)
    {
        ...
        out_emission = texture(u_sampler_2D[nonuniformEXT(material.baseTextureIndex + EMMISSION)], vertex_in.tex_coord);
//        out_emission.rgb *= 10.0 * out_emission.a;
    }

    vec3 normal = normalize(2.0 * (texture(u_sampler_2D[nonuniformEXT(material.baseTextureIndex + NORMAL)],
